...
class FloorsValue(FlaskForm):
    f1 = BooleanField(label=u'не первый', default=False)
...
class PeriodValue(FlaskForm):
    ...
    p2 = BooleanField(label=u'помесячно', default=False)
...
class PurchaseSearchForm(FlaskForm):
    ...
    rooms = FormField(RoomsValue, separator=' ')

    nfloor = FormField(FloorsValue, separator=' ')
...
class PurchaseCreateForm(FlaskForm):
    type = IntegerField(
        'Тип жилья: ',
        [
